...
async def _search_keyword(
    ...
    async def _native() -> list[dict]:
        """飞牛内置原生直连通道（网易云 + 酷我并发直连，不依赖可选容器）。

        可选音源容器（musicdl/musicbox/lxmusic）未部署时，_mb/_mdl/_lx 恒为空，
        会导致每日推荐检索不到任何曲目；此处复用飞牛内置的联想/模糊匹配能力兜底。
        """
        try:
            import features as _feats
            res = await _feats.get_search_suggestions(keyword)
        except Exception as e:
            logger.debug("native recommend search failed: %s", e)
            return []
        ...
        out: list[dict] = []
        for it in cands:
            if not isinstance(it, dict) or it.get("is_local"):
                continue
    ...
    jobs = [asyncio.create_task(coro) for coro in (_native(), _mb(), _mdl(), _lx())]
    try:
