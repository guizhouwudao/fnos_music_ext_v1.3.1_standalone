...
class State:
    ...
    def restore(self):
        with self.lock():
            ...
            if t['kind'] == 'official':
                if u['kind'] != 'absent':
                    # 若官方已经在 target 运行，且 upstream 残留了旧 socket，清理残留的 upstream socket
                    if u['kind'] in ('stale', 'unknown', 'official'):
                        try:
    ...
    def publish(self, staged, proxy):
        with self.lock():
            data = self.load()
            t, u = snapshot(self.target), snapshot(self.upstream)
            # 若官方进程重启后在 target 生成了新的 official socket，而 upstream 仍残留旧 socket，先清理 stale upstream
            if t['kind'] == 'official' and u['kind'] in ('stale', 'absent'):
                if u['kind'] == 'stale':
                    try:
                        os.unlink(self.upstream)
                    except OSError:
            ...
            if t['kind'] == 'official' and u['kind'] == 'absent':
                ...
                if prior and prior != u:
                    pass  # 允许官方进程重启后更新其身份记录
                self.remove_owned(self.target, data.get('proxy'))
                data.update(official=u, proxy=proxy)
                self.save(data)
            else:
                raise Unsafe(f'ambiguous/live socket topology: t={t.get("kind")} u={u.get("kind")}; nothing removed')
